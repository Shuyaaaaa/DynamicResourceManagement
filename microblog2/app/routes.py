...
# Token = data['Token']
# s3 = boto3.client('s3', aws_access_key_id=AccessKeyId, aws_secret_access_key=SecretAccessKey, aws_session_token=Token)
s3 = boto3.client('s3')
...
def upload():
    ...
    if filename != '':
        ...
        s3.upload_file(os.path.join(PROJECT_ROOT, 'static/image/'+filename), app.config['BUCKET_NAME'], 'static/image/{}'.format(filename), ExtraArgs={'ACL': 'public-read'})
    ...
    db.session.add(u)
    db.session.commit()
    os.remove(os.path.join('app/static/output/' + filename))
    os.remove(os.path.join('app/static/image/' + filename))
        ...
        for uploaded_file in filelist:
            ...
            if filename != '':
                ...
                if file_ext not in app.config['UPLOAD_EXTENSIONS']:
                    ...
                    uploaded_file.save(os.path.join('app/static/image', filename))
                    s3.upload_file(os.path.join(PROJECT_ROOT, 'static/image/' + filename), app.config['BUCKET_NAME'],
                                   'static/image/{}'.format(filename), ExtraArgs={'ACL': 'public-read'})
            else:
                response = jsonify({"error": {"code": 400, "message": "Error message!"}})
                response.status_code = 400
                return response
            ...
            num_unmask = 0
            image_type = 0
            output_info = inference(os.path.join(PROJECT_ROOT, 'static/image/' + filename),
                                    os.path.join(PROJECT_ROOT, 'static/output/' + filename),
                                    filename,
                                    show_result=False, target_shape=(360, 360))
            num_face = len(output_info)
            for i in range(num_face):
