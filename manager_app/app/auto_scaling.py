...
def auto_handler():
    ...
    if(manager.compare_inst()):
        print('All instances are healthy! Now we need to compare the thresholds!')
        ...
        if average_uti < threshold_min and len(instance_id) > 1:
            ...
            if remove_inst_num > 0 and (len(instance_id) - remove_inst_num) >= 1: # Minimum number of workers is 1
                ...
                print('IDs of the instances to be auto removed: ', id_to_remove)

                for id in id_to_remove:
                    manager.inst_remove(id)
    else:
        print('There exists unhealthy instance! Stopped Auto Scaling!')
        return
